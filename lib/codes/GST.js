...
exports.Decoder = function(id) {
    ...
    this._calculateRms = function(sigmas){
        var sum = 0;
        sigmas.forEach(function(sigma) {
            sum += Math.pow(sigma, 2);
        });
        return Math.sqrt(sum/sigmas.length);
    };
    ...
    this.parse = function(tokens) {
        ...
        var data = {
            ...
            altitudeError: Helper.parseFloatX(tokens[8]) //vrms
        };

        // See http://gis.stackexchange.com/a/218024/86167
        data.drms = this._roundRms(this._calculateRms([data.latitudeError, data.longitudeError]));
        data.hrms = this._roundRms(2*this._calculateRms([data.latitudeError, data.longitudeError])); //2drms
        data['3drms'] = this._roundRms(3*this._calculateRms([data.latitudeError, data.longitudeError]));
        data.rms3d = this._roundRms(this._calculateRms([data.latitudeError, data.longitudeError, data.altitudeError]));
